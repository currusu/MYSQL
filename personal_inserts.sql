...
#las respectivas tablas de empleados.#Acá lo que pedía era modificar momentáneamente el nombre de las columnas.
...
SELECT sal_emp, comision_emp FROM empleadosdepartamentosempleados
WHERE id_depto = 2000
...
SELECT (sal_emp + comision_emp ) AS 'salario_total' FROM empleados #El nombre nuevo temporal de la columna va con comillas simples
WHERE id_depto = 3000
ORDER BY nombre ASC;
...
SELECT sal_emp, comision_emp, (sal_emp + comision_emp), nombre FROM empleados
...
empleados.id_depto = departamentos.id_depto # Acá saqué promedio de todos los salarios , y después usé la clave foránea para relacionar las dos tablas
GROUP BY empleados.id_depto; #Acá usé empleados porque mi selección es con la tabla empleados
...
SELECT D.id_depto, COUNT(E.id_depto) FROM departamentos D #Esta D es un objeto que va a permitir usar las columnas de la tabla departamentos mediante un llamado
INNER JOIN empleados E ON #Con la cláusula Inner Join relaciono las dos tablas
E.id_depto = D.id_depto #Acá comparo la primary key de departamentos con la foreign key de empleados
GROUP BY E.id_depto #Acá agrupo el número de empleados
HAVING COUNT(E.id_depto) > 3; #Acá muestro los departamentos que tienen mas de tres empleados
...
select id_depto, d.nombre_depto, count(*) as 'Num empleados' #Acá selecciono el nombre y el id de los empleados desde la tabla departamentos y pido los cuente
...
SELECT D.id_depto, COUNT(E.id_depto) FROM empleados E #Esta D es un objeto que va a permitir usar las columnas de la tabla departamentos mediante un llamado
RIGHT JOIN departamentos D ON #Con la cláusula Inner Join relaciono las dos tablas
E.id_depto = D.id_depto #Acá comparo la primary key de departamentos con la foreign key de empleados
GROUP BY E.id_depto #Acá agrupo el número de empleados
HAVING COUNT(E.id_depto) = 0; #Acá muestro los departamentos que tienen mas de tres empleados
