...
SELECT nombre, precio ,AVG(precio), ROUND(AVG(precio)) FROM producto;
 

#5. Lista el código de los fabricantes que tienen productos en la tabla producto.
SELECT fabricante.codigo FROM fabricante
INNER JOIN producto ON
fabricante.codigo = producto.codigo;
...
SELECT p.nombre, MIN( p.precio), f.nombre FROM producto p, fabricante f
WHERE f.codigo = p.codigo_fabricante;

#4. Devuelve una lista de todos los productos del fabricante Lenovo.

SELECT p.*, f.nombre #Esto indica que los datos son de la tabla producto
FROM producto p 
...
INNER JOIN fabricante f
ON f.codigo = p.codigo_fabricante
WHERE f.nombre IN ('Asus','HewlettSTXPackard');
...
#Consultas Multitabla fabricanteproducto
...
#fabricanteproductonúmero de productos que el fabricante Lenovo